...
async function renderApp() {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        html += `<tr><td>${moisLabels[i]}</td><td><input type="number" value="${row.ca}" onchange="updateEntry(${i}, this.value, ${row.frais})"></td><td>${tvaMois.toFixed(2)} €</td><td>${urssaf.toFixed(2)} €</td><td><input type="number" value="${row.frais}" onchange="updateEntry(${i}, ${row.ca}, this.value)"></td><td style="font-weight:bold; color:#27ae60">${net.toFixed(2)} €</td></tr>`;
    }

    document.getElementById('tbody').innerHTML = html;
    document.getElementById('tfoot').innerHTML = `<tr style="background:rgba(0,0,0,0.05); font-weight:bold;"><td>TOTAL</td><td>${tCA.toFixed(2)} €</td><td>${tTVA.toFixed(2)} €</td><td>${tURSSAF.toFixed(2)} €</td><td>${tFrais.toFixed(2)} €</td><td>${tNet.toFixed(2)} €</td></tr>`;
    drawChart(netArray);
}
...
function drawChart(data) {
    ...
    window.chart = new Chart(ctx, {
        type: 'line',
        data: { labels: moisLabels.map(m => m.substring(0,3)), datasets: [{ label: 'Net (€)', data: data, borderColor: '#27ae60', backgroundColor: 'rgba(39,174,96,0.1)', fill: true, tension: 0.3 }] },
